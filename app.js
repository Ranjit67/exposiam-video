...
// group video stream
//host Speaker vab
const hostSpeakerUidTOId = {};
const hostSpeakerIdToUid = {};
//host speaker room
const hostSpeakerMemberRoom = {};
const hostSpeakerUserRoom = {};
// speaker member room
const speakerMemberUidToId = {};
const speakerMemberIdToUid = {};
const speakerMemberConnectedTo = {};
// user
const AUserUidToId = {};
const AUserIdToUid = {};
...
io.of("/group").on("connection", (socket) => {
  ...
  socket.on("host speaker join", (payload) => {
    try {
      ...
      hostSpeakerUidTOId[hostSpeaker] = socket.id;
      hostSpeakerIdToUid[socket.id] = hostSpeaker;
      //createRoom
      hostSpeakerMemberRoom[hostSpeaker] = [];
      hostSpeakerUserRoom[hostSpeaker] = [];
    } catch (error) {
      console.log(error);
    }
  });
  // speaker member join
  socket.on("Speaker member join", (payload) => {
    try {
      const { speakerMember, hostId, name } = payload;
      if (hostSpeakerUidTOId[hostId]) {
        console.log(hostSpeakerUidTOId[hostId]);
        socket
          .to(hostSpeakerUidTOId[hostId])
          .emit("speaker member send request", {
            name,
            speakerMemberUid: speakerMember,
            soId: socket.id,
          });
        // speakerMemberUidToId[speakerMember] = socket.id;
      } else {
        console.log(
          "we are nothing to keep any record of speaker member because early joining."
        );
      }
    } catch (error) {
      console.log(error);
  ...
  socket.on("host_accept", (payload) => {
    const { soId, speakerMemberUid, hostUid } = payload;
    // host keep recoard all of the speakerMember
    console.log(payload);
    // hostSpeakerMemberRoom
    // socket.to();
    // socket.emit("for create peer", { soId, speakerMemberUid });
    //
  });
});
